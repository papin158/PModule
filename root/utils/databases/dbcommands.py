...
class PUsers(DataBase):
    ...
    async def create_table(self):
        query = '''CREATE TABLE IF NOT EXISTS public."PUsers"( 
                   ...
                   FOREIGN KEY (user_group) REFERENCES public."SGroups"(user_group) ON DELETE SET NULL
                   );
                '''

        await self.connector.execute(query)

    async def add(self, user: aiogram.types.User, user_group: str):
        query = '''INSERT INTO public."PUsers" (user_id, user_fullname, user_group)
                   VALUES ($1, $2, $3)
                   ON CONFLICT (user_id) DO UPDATE SET user_group=excluded.user_group, user_fullname=excluded.user_fullname
                   RETURNING *;
                '''
        return await self.connector.fetch(query, user.id, user.full_name, user_group)

    async def get(self, user_group, what_select: str = '*'):
        query = f'''SELECT {what_select} FROM public."PUsers" WHERE user_group=$1;
                '''
